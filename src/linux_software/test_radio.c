...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    unsigned int bytes_transferred = 2048 * 4; // B
    float time_spent = stop_time - start_time; // cycles
    throughput = (bytes_transferred / time_spent) * (125000000 / 1e6);
    printf("You transferred %u bytes of data in %f seconds\n",bytes_transferred,(stop_time - start_time)/125000000.0);
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 10 Allison Kao - Custom Peripheral Demonstration\n\r");
